...
RSpec.describe "phone_numbers/edit", :type => :view do
  ...
  let(:phone_number) { PhoneNumber.create(contact_id: person.id, contact_type: "Person", number: "111-1111")}
  before(:each) do
  ...
  it "shows the contact's name in the title" do
    render
    assert("h1", text: "Editing Phone Number for #{phone_number.contact.first_name} #{phone_number.contact.last_name}")
  end
end
